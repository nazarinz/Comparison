...
with tab3:
    st.caption("Upload **SAP Excel (.xlsx)** saja. Dashboard menampilkan KPI & grafik berbasis kolom SAP.")

    sap_dash = st.file_uploader("📤 Upload SAP Excel untuk Dashboard (.xlsx)", type=["xlsx"], key="sap_dash_upload")

    if sap_dash:
        try:
            sap_df = load_sap(read_excel_file(sap_dash))
            ...
            with st.sidebar:
                ...
                date_axis_col = st.selectbox(
                    ...
                    options=[
                        c for c in ["CRD","LPD","PD","PO Date","Document Date","PSDD","FPD","PODD","Actual PGI"]
            ...
            if sd and ed:
                df = df[
                    (df[date_axis_col] >= pd.Timestamp(sd)) &
                    (df[date_axis_col] <= pd.Timestamp(ed) + pd.Timedelta(days=1) - pd.Timedelta(seconds=1))
                ]
            ...
            for col, vals in [
                ("Segment", segs), ("Site", sites), ("Brand FTY Name", brands),
                ("Ship-to Country", shipc), ("Merchandise Category 2", cat2),
            ...
            with colA:
                ...
                if "Ship-to Country" in df.columns:
                    top_c = (
                        df.groupby("Ship-to Country", as_index=False)["Quantity"]
                        .sum().sort_values("Quantity", ascending=False).head(10)
                    )
                    st.bar_chart(top_c.set_index("Ship-to Country")["Quantity"])
                else:
            ...
            with colB:
                ...
                if "Model Name" in df.columns:
                    top_m = (
                        df.groupby("Model Name", as_index=False)["Quantity"]
                        .sum().sort_values("Quantity", ascending=False).head(10)
                    )
                    st.bar_chart(top_m.set_index("Model Name")["Quantity"])
                else:
            ...
            if cols_delay:
                for dc in cols_delay:
                    cnt = (
                        df[dc].astype(str).str.upper()
                        .replace({"NAN": "", "NULL": "", "--": "", "NAT": "", "N/A": ""})
                    )
            ...
            st.download_button(
                "Download CSV",
                data=df.drop(columns=["__Value__"], errors="ignore").to_csv(index=False).encode("utf-8"),
                file_name=f"SalesAnalytics_SAP_Filtered_{today_str_id()}.csv",
                mime="text/csv", use_container_width=True,
            )
            try:
                excel_bytes = _export_excel_styled(
                    df.drop(columns=["__Value__"], errors="ignore"),
                    sheet_name="Sales Analytics",
                )
                st.download_button(
                    "Download Excel (styled)", data=excel_bytes,
                    file_name=f"SalesAnalytics_SAP_Filtered_{today_str_id()}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    use_container_width=True,
                )
            except Exception as ex_excel:
